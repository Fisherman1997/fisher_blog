...
import { Edit, Expand, Fold, SwitchButton, User, UserFilled } from '@element-plus/icons-vue'
import { deleteStoreRouter } from '@/routers/router.store'
import { delStorageItem } from '@/utils/storage'
import { useUserStore } from '@/store/user.store.js'
import customDialog from '@/components/customDialog/CustomDialog'
import InsertAndUpdateUser from '@/views/userManage/modula/InsertAndUpdateUser.vue'
import ChangePassword from '@/views/userManage/modula/ChangePassword.vue'
...
const routeList = reactive<{ list: listTpye[] }>({ list: [] })
...
const emit = defineEmits<{
    (e: 'changeLfState', payload: boolean): void
}>()
const changeLfState = (value: boolean) => emit('changeLfState', value)
...
watch(
    ...
    () => {
        ...
        const { path, meta } = router
            .getRoutes()
            .find((item) => item.path === route.path) as RouteRecordNormalized
        routeList.list.push({ path, title: meta.title as string })
    },
)
